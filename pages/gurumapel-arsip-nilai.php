...
$page_nav["kbm"]["sub"]["arsipnilai"]["active"] = true;
include("inc/nav.php");
...
switch ($sub)
{
	case "tampil":default:
		...
		while ($HTA=mysql_fetch_array($QTA)){
			$Sel=$HTA['tahunajaran']==$_GET['thnajaran']?"selected":"";
			$InputTA.="<option $Sel value={$HTA['tahunajaran']}>{$HTA['tahunajaran']}</option>";
		}
		$QGg=mysql_query("select * from ak_semester");
		while ($HGg=mysql_fetch_array($QGg)){
			$Sel=$HGg['ganjilgenap']==$_GET['gg']?"selected":"";
			$DtGG.="<option $Sel value={$HGg['ganjilgenap']}>{$HGg['ganjilgenap']}</option>";
		}
//===========================================================================================================[FORM ADMIN NYARI DATA]
		...
		<form action='?page=$page' method='post' name='frmaing' class='form-horizontal' role='form'>
			<div class='form-group'>
				...
				<div class='col-sm-5'>
					<select name=\"txtThnAjar\" class='input-sm form-control' onchange=\"document.location.href='?page=$page&thnajaran='+document.frmaing.txtThnAjar.value\">
			...
			<div class='form-group'>
				...
				<div class='col-sm-4'>
					<select name=\"txtGG\" class='input-sm form-control' onchange=\"document.location.href='?page=$page&thnajaran='+document.frmaing.txtThnAjar.value+'&gg='+document.frmaing.txtGG.value\">
						<option value=''>Pilih</option>
						$DtGG
					</select>
				</div>
...
//=============================================================[CEK KBM PER GURU]
		$QGuru=mysql_query("select kd_guru,count(*) as jml from gmp_ngajar where thnajaran='$thnajaran' and ganjilgenap='$gg' and kd_guru='$IDna' group by kd_guru");
		$TampilGuru.="
		<div class='well no-padding'>
			...
			<tr>
				...
				<td class=text-center bgcolor='DDDDDD'><strong>$JmlGuru</strong></td>
			</tr>
			</table>
		</div>";
		$DtKBMGuru.=JudulKolom("KBM Per Tenaga Pendidik","");
		if($JmlData02==0){$DtKBMGuru.="<h1 class='text-center text-danger'><strong><i class='fa fa-exclamation-triangle fa-lg'></i></h1><h4 class='text-center text-info'>Data Kosong.<br>Silakan Pilih Data !!</strong></h4>";}else{$DtKBMGuru.="$TampilGuru";}
		if($JmlData01==0 && $JmlData02==0) {
			if($thnajaran==""){
				$DtKosong="<h1 class='text-center text-danger'><strong><i class='fa fa-exclamation-triangle fa-lg'></i></h1><h4 class='text-center text-info'>Silakan Pilih Tahun Ajaran !!</strong></h4>";
			}else if($gg==""){
				$DtKosong="<h1 class='text-center text-danger'><strong><i class='fa fa-exclamation-triangle fa-lg'></i></h1><h4 class='text-center text-info'>Silakan Pilih Semester !!</strong></h4>";
			}else
			{
				$DtKosong="<h1 class='text-center text-danger'><strong><i class='fa fa-exclamation-triangle fa-lg'></i></h1><h4 class='text-center text-info'>Data Belum Ada!!</strong></h4>";
			}
			$Show.=DuaKolomSama($NyariData,$DtKosong);
		}
		else {
			$thnajaran=(isset($_GET['thnajaran']))?$_GET['thnajaran']:"";
			$gg=(isset($_GET['gg']))?$_GET['gg']:"";
			...
			while($HNgajar=mysql_fetch_array($QNgajar))
			{
				...
				<tr>
					...
					<td>{$HNgajar['thnajaran']}</td>
					<td>{$HNgajar['semester']}</td>
					<td>{$HNgajar['nama_mapel']}</td>
					<td>{$HNgajar['nama_kelas']}</td>
					<td>{$HNgajar['kkmpeng']}</td>
					<td>{$HNgajar['kkmket']}</td>
					<td><a href='?page=$page&amp;sub=cekkikd&amp;Kode={$HNgajar['id_ngajar']}&thnajaran=$thnajaran&gg=$gg' title='Arsip KBM'><i class='fa fa-folder-open-o fa-border txt-color-red'></i> <span class='hidden-mobile'>Data KBM</span></a> | <a href='?page=$page&amp;sub=arsipnilai&amp;kbm={$HNgajar['id_ngajar']}&thnajaran=$thnajaran&gg=$gg' title='Arsip Nilai'><i class='fa fa-graduation-cap fa-border txt-color-red'></i> <span class='hidden-mobile'>Data Nilai</a></span></td>
				</tr>";
				$no++;
			}
				...
				<table id='dt_basic' class='table table-striped table-bordered table-hover table-condensed' width='100%'>
					<thead>
						<tr>
							...
							<th class='text-center' data-hide='phone,tablet'>KKM P</th>
							<th class='text-center' data-hide='phone,tablet'>KKM K</th>
			...
			$Show.=DuaKolomD(8,$NyariData,4,$DtKBMGuru);
			$Show.=KolomPanel($ShowArsip);
		}
	...
	//KD SIKAP ============================================================================
		...
		else{$ShowSikap.=nt("informasi","Tidak Ada KIKD SIKAP");}	
	//KD PENGETAHUAN =============================================================================
		$noP=1;
		...
		else{$ShowPengetahuan.=nt("informasi","Tidak Ada KIKD PENGETAHUAN");}
	//KD KETERAMPILAN =============================================================================
		$noK=1;
		...
		else{$ShowKet.=nt("informasi","Tidak Ada KIKD SIKAP");}
	...
	case "arsipnilai":
		...
		$Show.=$ReviewNilai;
		echo IsiPanel($Show);
		//echo WidgetToBack($Show,"Review","Nilai $Kls","reorder","#ReviewNilai");
	break;
}
echo '</div>';

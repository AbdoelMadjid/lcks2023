...
switch($sub)
{
	case "tampil":default:
		...
		if($jmlKDS==0 && $jmlKDP==0 && $jmlKDK==0){
			...
			$Show.=JudulKolom("Proses Nilai Sikap","edit");
			$Show.=nt("peringatan","<strong>Kompetensi Dasar </strong> belum di pilih, silakan isi dulu <br><a href='?page=gmp-ki-kd' class='btn btn-default btn-xs'>Kompetensi Dasar Mata Pelajaran</a>");
		}
		else{
			$QSikap=mysql_query("select * from n_sikap where kd_ngajar='$kbm'");
			$JmlhData=mysql_num_rows($QSikap);
			...
			if($JmlhData==$JDT){
//============================================================================[ PROSES EDIT DATA NILAI SIKAP ]
				$Q=mysql_query("select n_sikap.*,siswa_biodata.nm_siswa from n_sikap,siswa_biodata where n_sikap.nis=siswa_biodata.nis and n_sikap.kd_ngajar='$kbm' order by siswa_biodata.nm_siswa,n_sikap.nis");	
				...
				while($Hasil=mysql_fetch_array($Q)){
					$ShowNPData.="<tr>
						...
						<td>{$Hasil['nm_siswa']}</td>
							<input class='form-control input-sm' type='hidden' value='".$Hasil['kd_skp']."' name='txtKNKDS[$i]'>
						...
						<td class='text-center'>".FormCR("smart","","txtSpritual[$i]",$Hasil['spritual'],$Ref->NilaiSikap,"","")."</td>
						<td class='text-center'><textarea name='txtDescSSpr[$i]' class='form-control input-sm'>".$Hasil['desk_spritual']."</textarea></td>
						<td class='text-center'>".FormCR("smart","","txtSosial[$i]",$Hasil['sosial'],$Ref->NilaiSikap,"","")."</td>
						<td class='text-center'><textarea name='txtDescSSos[$i]' class='form-control input-sm'>".$Hasil['desk_sosial']."</textarea></td>
					</tr>";
					$i++;
				}
				$Show.=ButtonKembali2("?page=gurumapel-data-penilaian","style='margin-top:-10px;'");
				$Show.="<a href=\"javascript:;\" onClick=\"window.open('./pages/excel/ekspor-nilai.php?eksporex=nilai-sikap&kbm=$kbm&mapel={$HDT['nama_mapel']}&kls={$HDT['nama_kelas']}&komp=sikap')\" class='btn btn-default btn-sm pull-right' style='margin-top:-10px;margin-right:10px;' title='Download Nilai'><i class='fa fa-download'></i> <span class='hidden-xs'>Download Nilai</span></a>";
				$Show.="
				<div class='btn-group pull-right'>
					<button class='btn btn-default dropdown-toggle' data-toggle='dropdown' style='margin-top:-10px;margin-right:10px;'>
						<span class='label label-primary'>K1K2</span> <span class='hidden-xs'> Sikap </span><span class='caret'></span>
					</button>
					<ul class='dropdown-menu'>
						<li><a href='?page=gurumapel-nilai-sikap&kbm=$kbm'>Sikap <i class='fa fa-check'></i></a>
				...
				$Show.=JudulKolom("Edit Nilai Sikap","edit");
				$Show.=KotakWell("
				...
				$Show.="
				$Ulangi
				<form action='?page=$page&sub=simpanedit' method='post' name='FrmSikap' id='FrmSikap' role='form'>
				...
				<table id='dt_basic' class='table table-striped table-bordered table-hover table-condensed' width='100%'>
					<thead>
						<tr>
							...
							<th class='text-center' data-hide='phone,tablet'>Spritual</th>
							<th class='text-center' data-hide='phone,tablet'>Catatan Spritual</th>
							<th class='text-center' data-hide='phone,tablet'>Sosial</th>
							<th class='text-center' data-hide='phone,tablet'>Catatan Sosial</th>
						</tr>
					</thead>
				...
				{
					$Show.="<a href='?page=$page&sub=HapusNilai&kbm=$kbm' data-action='hapusnilais' data-hapusnilais-msg=\"Apakah Anda yakin akan mengapus nilai sikap kbm <strong>$kbm</strong>?\" class='btn btn-danger btn-sm'>Hapus Nilai</a> ".bSubmit("SaveUpdate","Update");
				}
				$Show.="</div></form>";
			}
			else if($JmlhData>$JDT){
//============================================================================[ PROSES HAPUS DATA DOUBLE ]
				$Q=mysql_query("select n_sikap.*,siswa_biodata.nm_siswa from n_sikap,siswa_biodata where n_sikap.nis=siswa_biodata.nis and n_sikap.kd_ngajar='$kbm' order by siswa_biodata.nm_siswa,n_sikap.nis");	
				$i=1;			
				while($Hasil=mysql_fetch_array($Q)){
					$ShowNPData.="
					<tr>
						...
						<td>{$Hasil['nm_siswa']}</td>
							...
							<input class='form-control input-sm' type='hidden' value='".$kbm."' name='txtKBM[$i]'>
							<input class='form-control input-sm' type='hidden' value='".$Hasil['nis']."' name='txtNIS[$i]'>
						<td>".$Hasil['spritual']."</td>
						<td>".$Hasil['desk_spritual']."</td>
						<td>".$Hasil['sosial']."</td>
						<td>".$Hasil['desk_sosial']."</textarea></td>
						<td><label class='checkbox'><input type='checkbox' id='cekbox' name='cekbox[]' value='{$Hasil['kd_skp']}'><i></i></label></td>
					</tr>";
					$i++;
				}
				$Show.=ButtonKembali2("?page=gurumapel-data-penilaian","style='margin-top:-10px;'");
...
//============================================================================[ PROSES TAMBAH DATA NILAI SIKAP ]
				$Q=mysql_query("
				select 
				gmp_ngajar.id_ngajar,
				gmp_ngajar.kd_guru,
				...
				$Show.=ButtonKembali2("?page=gurumapel-data-penilaian","style='margin-top:-10px;'");
				$Show.="<a href='?page=$page&sub=upload_nilai&guru=$kdguru&thnajaran=$thajar&gg=$smester&kbm=$kbm' class='btn btn-default btn-sm pull-right' style='margin-top:-10px;margin-right:10px;'> <i class='fa fa-upload'></i>  <span class='hidden-xs'>Upload Nilai Sikap</span></a>";
				$Show.="
				<div class='btn-group pull-right'>
					<button class='btn btn-default dropdown-toggle' data-toggle='dropdown' style='margin-top:-10px;margin-right:10px;'>
						<span class='label label-primary'>K1K2</span> <span class='hidden-xs'> Sikap </span><span class='caret'></span>
				...
				$Show.=JudulKolom("Tambah Nilai Sikap","plus");
				$Show.=KotakWell("
				<dl class='dl-horizontal' style='margin-bottom:0px;'>
				  <dt>Tahun Pelajaran : </dt><dd>{$HDT['thnajaran']}</dd>
				  <dt>Semester : </dt><dd>{$HDT['semester']} ({$HDT['ganjilgenap']})</dd>
				...
				$Show.="
				<form action='?page=$page&sub=simpantambah' method='post' name='FrmSikap' id='FrmSikap' role='form'>
				<div class='well no-padding'>
				<table id='dt_basic' class='table table-striped table-bordered table-hover table-condensed' width='100%'>
					<thead>
						<tr>
				...
				$Show.="<div class='form-actions'>".bSubmit("SaveNSP","Simpan")."</div></form>";
			}
		}
		echo IsiPanel($Show);
	...
	case "simpanedit":
		foreach($_POST['txtKNKDS'] as $i => $txtKNKDS){
			$txtKBM=$_POST['txtKBM'][$i];
			$txtNIS=$_POST['txtNIS'][$i];
			$txtSpritual=$_POST['txtSpritual'][$i];
			$txtSosial=$_POST['txtSosial'][$i];
			$txtDescSSpr=$_POST['txtDescSSpr'][$i];
			$txtDescSSos=$_POST['txtDescSSos'][$i];
			mysql_query("UPDATE n_sikap SET kd_ngajar='$txtKBM',nis='$txtNIS',spritual='$txtSpritual',sosial='$txtSosial',desk_spritual='$txtDescSSpr', desk_sosial='$txtDescSSos' WHERE kd_skp='$txtKNKDS'");
			echo ns("Ngedit","parent.location='?page=$page&kbm=$txtKBM'","Nilai Sikap dengan Kode KBM <strong>$txtKBM </strong>");
		}
	break;
	...
	case "simpantambah":
		foreach($_POST['txtKBM'] as $i => $txtKBM){
			...
			echo ns("Nambah","parent.location='?page=$page&kbm=$txtKBM'","Nilai Sikap dengan Kode KBM <strong>$txtKBM </strong>");
		}
	break;
	...
	case "HapusNilai":
		...
		echo ns("Hapus","parent.location='?page=$page&kbm=$kbm'","Nilai Sikap dengan Kode KBM <strong>$kbm </strong>");
	break;
	...
	case "hapusbanyak":
		...
		if ($cekbox) {
			foreach ($cekbox as $value) {
				mysql_query("DELETE FROM n_sikap WHERE kd_skp='$value'");
				echo $value;
				echo ",";
			}
			echo ns("Hapus","parent.location='?page=$page&kbm=$kbm'","Nilai Double");
		}  else {
			echo ns("BelumMilih","parent.location='?page=$page&kbm=$kbm'","");
		}
	break;
	...
	case "upload_nilai":
		...
		ID KBM <strong>{$H['id_ngajar']}</strong><br>Tahun Ajaran <strong>{$H['thnajaran']}</strong><br>Semester <strong>{$H['semester']}</strong><br>Kelas <strong>{$H['nama_kelas']}</strong><br>Mata Pelajaran <strong>{$H['nama_mapel']}</strong>");
	
		$UloadPerNilai.=JudulDalam("Nilai K1K2 - Sikap","");
		$UloadPerNilai.="
...
$(document).ready(function() {
			...
			$.SmartMessageBox(
				{
					...
					"content":a.data("hapusnilais-msg"),
					"buttons":"[No][Yes]"
				},function(a){
	...
	$.root_.on("click",'[data-action="hapusnilais"]',function(b){var c=$(this);a.hapusnilais(c),b.preventDefault(),c=null});

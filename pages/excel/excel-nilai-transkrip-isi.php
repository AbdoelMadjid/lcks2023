...
$DMP=mysql_query("select * from matapelajaran where kode_mapel='$mp'");
$HMP=mysql_fetch_array($DMP);
$NamaMapel=$HMP['nama_mapel'];

$DKelas=mysql_query("select * from kelas where tahunajaran='$thnajaran' and kode_pk='$pk' and kode_kelas='$kls'");
...
if($jn=='UNPK' || $jn=='US-DPK' || $jn=='US-PK' || $jn=='UJIKOM' || $jn=='NL'){
	$result=mysql_query("
		...
		FROM 
		ujian_usukk 
		INNER JOIN ak_kelas ON ujian_usukk.kelas=ak_kelas.nama_kelas and ujian_usukk.thajaran=ak_kelas.tahunajaran 
		WHERE ak_kelas.tahunajaran='$thnajaran' 
		ORDER BY  ak_kelas.kode_kelas,ujian_usukk.nm_siswa");
}
else if($jn=='UN'){
	$result=mysql_query("
		SELECT 
...
else if($jn=='US')
{
	...
	and ak_matapelajaran.kode_mapel='$mp' 
...
while($row=mysql_fetch_array($result)) {  
	...
	if($jn=="UN"){
		...
		$QNilUS=mysql_query("SELECT * FROM n_t_us WHERE thnajaran='$thnajaran' AND kode_mapel='$mp' AND kls='$kls' and nis='{$row['nis']}'");
		$HNilUS=mysql_fetch_array($QNilUS);

		$objPHPExcel->getActiveSheet()->setCellValue('D'.$kolomna, $row['kode_mapel']);
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$kolomna, $row['nis']);
		$objPHPExcel->getActiveSheet()->setCellValue('F'.$kolomna, $row['nm_siswa']);  
		$objPHPExcel->getActiveSheet()->setCellValue('G'.$kolomna, $HNilUS['nust']);  
		$objPHPExcel->getActiveSheet()->setCellValue('H'.$kolomna, $HNilUS['nusp']);  
